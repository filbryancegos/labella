...
function cms_options($params = array())
{
    ...
    if (is_admin() && !empty($params['id']) && isset($core_options)) {
        ...
        elseif ($pagenow=='post-new.php' || $pagenow=='post.php'){
            ...
            // Get value
            if(!empty($wp_pizzeria_meta->$params['id'])){
                $params['value'] = $wp_pizzeria_meta->$params['id'];
            } else {
                $params['value'] = null;
            }
...
class CsCoreControl
{
        ...
        <div class="images-field clearfix">
            ...
            foreach ($selected as $value):
                ...
                <li class="items" data-id="<?php echo esc_attr($value); ?>" style="background-image:url(<?php echo esc_url($attachment_image[0]);?>);background-size:cover;">
                    <i class="edit dashicons dashicons-plus-alt" title="<?php esc_html_e('Replace Image', 'wp_pizzeria'); ?>"></i>
                    <i class="remove dashicons dashicons-dismiss" title="<?php esc_html_e('Remove Image', 'wp_pizzeria'); ?>"></i>
                </li>
            <?php endif; endforeach; ?>
            <?php if($params['field'] != 'single'): ?>
                <li class="items" data-id=""><i class="add dashicons dashicons-plus-alt" title="<?php esc_html_e('Add Image', 'wp_pizzeria'); ?>"></i></li>
    ...
    public function save_meta_boxes($post_id)
	{
		...
		/* array cms meta */
        $wp_pizzeria_meta = array();
        
        /* find cms meta key. */
		foreach($_POST as $key => $value) {
			if(strstr($key, '_cms_')) {
			    $wp_pizzeria_meta[$key] = rawurlencode($value);
			}
		}
		/* update _cms_meta_data. */
		if(!empty($wp_pizzeria_meta)){
		  update_post_meta($post_id, '_cms_meta_data', json_encode($wp_pizzeria_meta));
		}
	}
}